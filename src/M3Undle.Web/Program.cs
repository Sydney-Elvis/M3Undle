...
builder.Services.AddSingleton<ILinuxResourceFileReader, LinuxResourceFileReader>();
builder.Services.AddSingleton<ResourceFactsService>();
builder.Services.AddSingleton<IResourceFactsProvider>(sp => sp.GetRequiredService<ResourceFactsService>());
builder.Services.AddHostedService(sp => sp.GetRequiredService<ResourceFactsService>());
...
static bool IsMediaSurfacePath(PathString path)
{
    return path.StartsWithSegments("/hdhr", StringComparison.OrdinalIgnoreCase)
           ...
           || path.Equals("/xmltv.php", StringComparison.OrdinalIgnoreCase)
